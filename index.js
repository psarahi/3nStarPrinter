...
socket.on("printFactura", (data) => {
  ...
  if (datosImprimir.sucursales !== "68559317a53a7aa6def1834f") {
    return;
  }
  try {
    ...
    device.open(function (error) {
      ...
      for (let index = 0; index < 2; index++) {
        printer
          ...
          .text(`RS: ${datosImprimir?.rsocial.toLocaleUpperCase()}`)
          .font("a")
          .align("CT")
          .encode("utf8")
          .size(0, 0)
          .text(datosImprimir.eslogan.toLocaleUpperCase())
...
socket.on("printOrdenTrabajo", (data) => {
  ...
  let { datosImprimir } = data;
  if (datosImprimir.sucursalId !== "68559317a53a7aa6def1834f") {
    return;
  }
  try {
    const device = new escpos.USB();
  ...
  `;
    ...
    device.open(function (error) {
      ...
      printer
        ...
        .text(`${datosImprimir.direccion}`)
        .text(`Tel: ${datosImprimir.telefono}  Edad: ${datosImprimir.edad}`)
        .text("")
        .text(
          `OD ${textValidator(datosImprimir.recetaOjoDerecho.esfera)
        ...
        .text(
          `Add:${textValidator(datosImprimir.recetaOjoDerecho.adicion)
            ? datosImprimir.recetaOjoDerecho.adicion
            : ""
          } DP:${textValidator(datosImprimir.recetaOjoDerecho.distanciaPupilar)
        ...
        .text("")
        .text(
          `OI ${textValidator(datosImprimir.recetaOjoIzquierdo.esfera)
        ...
        .text(
          `Add:${textValidator(datosImprimir.recetaOjoIzquierdo.adicion)
            ? datosImprimir.recetaOjoIzquierdo.adicion
            : ""
          } DP:${textValidator(datosImprimir.recetaOjoIzquierdo.distanciaPupilar)
